...
namespace assignfeedback_plaincomment\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $data = [
            ...
            'plaincomment' => 'privacy:metadata:plainpurpose'
        ];
        $collection->add_database_table('assignfeedback_plaincomment', $data, 'privacy:metadata:tablesummary');
    ...
    public static function export_feedback_user_data(assign_plugin_request_data $exportdata) {
        // Get that plain information and jam it into that exporter.
        $assign = $exportdata->get_assign();
        $plugin = $assign->get_plugin_by_type('assignfeedback', 'plaincomment');
        $gradeid = $exportdata->get_pluginobject()->id;
        $plaincomment = $plugin->get_feedback_plaincomment($gradeid);
        if ($plaincomment && !empty($plaincomment->plaincomment)) {
            $currentpath = array_merge(
                $exportdata->get_subcontext(),
                [get_string('privacy:plainpath', 'assignfeedback_plaincomment')]
            );

            $plaincomment->plaincomment = writer::with_context($assign->get_context())->rewrite_pluginfile_urls(
                $currentpath,
                ASSIGNFEEDBACK_PLAINCOMMENT_COMPONENT,
                ASSIGNFEEDBACK_PLAINCOMMENT_FILEAREA,
                $gradeid,
                $plaincomment->plaincomment
            );
            $data = (object) ['plaincomment' => $plaincomment->plaincomment];
            ...
            writer::with_context($exportdata->get_context())->export_area_files($currentpath,
                ASSIGNFEEDBACK_PLAINCOMMENT_COMPONENT, ASSIGNFEEDBACK_PLAINCOMMENT_FILEAREA, $gradeid);
        }
    }
    ...
    public static function delete_feedback_for_context(assign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, ASSIGNFEEDBACK_PLAINCOMMENT_COMPONENT,
            ASSIGNFEEDBACK_PLAINCOMMENT_FILEAREA);
    ...
    public static function delete_feedback_for_grade(assign_plugin_request_data $requestdata) {
        ...
        $fs = new \file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, ASSIGNFEEDBACK_PLAINCOMMENT_COMPONENT,
            ASSIGNFEEDBACK_PLAINCOMMENT_FILEAREA, $requestdata->get_pluginobject()->id);

        $DB->delete_records('assignfeedback_plaincomment', ['assignment' => $requestdata->get_assignid(),
    ...
    public static function delete_feedback_for_grades(assign_plugin_request_data $deletedata) {
        ...
        $DB->delete_records_select('assignfeedback_plaincomment', "assignment = :assignment AND grade $sql", $params);
    }
}
